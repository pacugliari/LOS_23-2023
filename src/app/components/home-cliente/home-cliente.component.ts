...
export class HomeClienteComponent implements OnInit {
  ...
  async actualizarUsuario(){
    ...
    this.usuario = usuarios.filter((usuario:any)=> usuario.id === usuarioLog.id)[0]
        ...
        if (userQR === QRs.IngresoLocal && this.usuario?.data?.enListaEspera !== "Espera") {
          ...
          await this.firestoreService.modificar(this.usuario,"usuarios")
            ...
            if(aux.id === idMesa && this.usuario.data.estadoMesa === EstadoMesa.NoVinculada){
              ...
              this.mensajesService.mostrar("ERROR","Usted ya vinculo la mesa","error");
            }else{
              this.mensajesService.mostrar("ERROR","Error la mesa escaneada no es la suya,el numero de mesa de usted es: "+aux.data.numeroMesa,"error");
            }
        }else if(this.usuario?.data?.enListaEspera === "Espera"){
          this.mensajesService.mostrar("ERROR","Usted esta en lista de espera,debe esperar que un metre le asigne mesa","error")
        }else{
          this.mensajesService.mostrar(
            'NO VALIDO',
